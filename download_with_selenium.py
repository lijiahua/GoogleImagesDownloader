...
def get_image_links(main_keyword, supplemented_keyword, link_file_path, num_requested=1000):
    ...
    print('Process-{0} add keyword {1} , got {2} image urls so far'.format(main_keyword, supplemented_keyword,
                                                                           len(img_urls)))
    print('Process-{0} totally get {1} images'.format(supplemented_keywords, len(img_urls)))
    driver.quit()
...
if __name__ == "__main__":
    ...
    for i in range(len(supplemented_keywords)):
        p.apply_async(get_image_links,
                      args=(main_keywords[0], supplemented_keywords[i], link_files_dir + supplemented_keywords[i]))
    p.close()
    p.join()
    ...
    # multiple processes
    p = Pool()  # default number of process is the number of cores of your CPU, change it by yourself
    for keyword in supplemented_keywords:
